  ...
  const onMessageReceived = (payload) => {
    ...
    switch (payloadData.status) {
      ...
      case "MESSAGE":
        ...
        const uniqueObjects = publicChats.filter(
          (obj, index, self) =>
            index ===
            self.findIndex((o) => getUniqueKey(o) === getUniqueKey(obj))
        );

        setPublicChats(uniqueObjects);

        console.log("unique " + JSON.stringify(uniqueObjects));
        break;
      case "SONDAGE":
        publicChats.push(payloadData);

        setPublicChats([...publicChats]);
        const getUniqueKey2 = (obj) => JSON.stringify(obj);

        // Filtering the array to remove duplicates
        const uniqueObjects2 = publicChats.filter(
          (obj, index, self) =>
            index ===
            self.findIndex((o) => getUniqueKey2(o) === getUniqueKey2(obj))
        );

        setPublicChats(uniqueObjects2);
    ...
    <div className="container">
      {userData.connected ? (
        <Navbar
          ...
          userBackendData={props.userBackendData}
        ></Navbar>
      ) : (
        ""
